...
let set_color set color =
  ...
  | "red" ->
      {set with red= set.red + num}
  | "blue" ->
      {set with blue= set.blue + num}
  | _ ->
      {set with green= set.green + num}

let parse_set str =
  str |> Str.split (Str.regexp ", ") |> List.fold_left set_color empty_set

let parse_sets str = str |> Str.split (Str.regexp "; ") |> List.map parse_set
...
let part2 games = games |> List.map power_of_game |> List.fold_left ( + ) 0
