...
let first_digit line = 
    let rec aux chars = match chars with
        ...
        | 'e' :: 'i' :: 'g' :: 'h' :: 't' :: _ -> 8
        | 'n' :: 'i' :: 'n' :: 'e' :: _ -> 9
        | c :: _ when is_digit c -> Option.value ~default:0 (Char.get_digit c)
        | _ :: rest -> aux rest 
        | [] -> 0 in
...
let part1 lines = lines
    |> List.map ~f:(fun line ->
        let rec first chars = match chars with
            | c :: _ when is_digit(c) -> Option.value ~default:0 (Char.get_digit c)
            | _ :: rest -> first rest 
            | [] -> 0 in
        (first (String.to_list line)) * 10 + (first (String.to_list (String.rev line)))
    )
    |> List.fold ~f:(+) ~init:0
;;

let part2 lines = lines
    |> List.map ~f:int_from_line
    |> List.fold ~f:(+) ~init:0
;;
